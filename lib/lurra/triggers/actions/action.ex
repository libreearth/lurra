defmodule Lurra.Triggers.Action do
  ...
  def run(actions, rule, payload, %Trigger{} = trigger) when is_list(actions) and is_map(rule) do
    Enum.each(actions, fn action -> apply(String.to_atom(action["module"]), :run, [action, rule, payload, trigger]) end)
  end

  @callback name()::binary
  @callback params()::[binary]
  @callback run(action:: map, rule::map, payload::any, trigger::map):: :ok
end
