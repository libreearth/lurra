...
let Chart = {
  ...
  getXAxis() {
    return d3.axisBottom(this.currentXScale).ticks(5).tickFormat(d3.timeFormat("%b %d %H:%M:%S"))
  },
  ...
  mounted(){
    ...
    this.handleEvent("window-changed-from-to", (data) => this.timeWindowChangedFromTo(data.from, data.to))
    ...
    this.handleEvent("toogle-lablogs", () => this.toggleLablogs())
    this.createChart()
    this.updateData()
  },

  toggleLablogs(){
    this.showLablogs = !this.showLablogs
    this.drawData()
  },
  ...
  timeWindowChangedFromTo(from, to){
    this.timeWidth = to - from
    this.toTime = to
    this.fromTime = from
    this.currentXScale = this.getXScale()
    this.updateDayRects()
